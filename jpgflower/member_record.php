...
<?php
 ...
 $sqlstr = "select * from transaction,record,product where transaction.mid='".$mid."' and transaction.tno=record.tno and product.pno=record.pno;";
 $result = mysqli_query($link,$sqlstr);
 
 $nrow = mysqli_num_rows($result);
 $i=1;
 echo"<div class='main'>";		
 echo"<table><tr><th>交易時間</th><td>商品名稱</td><td>交易編號</td><td>交易金額</td></tr>";
 while($i<=$nrow){
	 
  echo"<tr>";
  $record = mysqli_fetch_assoc($result);
 echo"<th>{$record['ttime']}</th>";
 echo"<td>{$record['pname']}</td>";
  echo"<td>{$record['tno']}</td>";
	 echo"<td>{$record['rprice']}</td>";
